...
router.get('/LC-Outstanding', (req, res, next) => {
    try {
        ...
        Query = `SELECT * FROM (
    SELECT 
        Any_value(y.spinner) AS spinner,
...
router.get('/LCOutstandingTotal', (req, res, next) => {
    try {
        ...
        Query = `SELECT  Round(sum(Receiptvalue) , 2)  as Receiptvalue , Round(sum(PendingLCValue),2) as PendingLCValue  , 
round( sum(PendingReceiptKgs),2 ) as PendingReceiptKgs ,
round( sum(receiptYarnKgs),2 ) as ReceiptKgs FROM (
    SELECT 
        Any_value(y.spinner) AS spinner,
        Any_value(y.lcNo) AS lcNo,
        Any_value(DATE_FORMAT(y.lcDate, "%Y-%m-%d")) AS lcDate,
