...
router.post('/linemachinelist-master', (req, res, next) => {
    try {
        ...
        var data = [];
        var headerQuery = "INSERT INTO tmp_linemachinelist_lines (lineid, headid,line , createdBy, orgId) values "
        var data = req.body.data;
        var i = 0;
        for (let datalist of data) {
...
router.get('/linemachinelist-list', (req, res, next) => {
    try {
        ...
        Query = `SELECT lm.*, 
       (SELECT json_arrayagg(json_object('line', lml.line, 'lineId', lml.linemasterid, 'hederid', lml.headid)) 
