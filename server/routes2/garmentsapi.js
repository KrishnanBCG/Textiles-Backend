...
router.get('/sewing-packing', (req, res, next) => {
    try {
        ...
        Query = `select * , date_format(packDate, '%d-%m-%Y') as packDate
...
router.post('/sewing-packing', (req, res, next) => {
    try {
        ...
        var packDate = req.body.packDate;
        var data = [];
        var headerQuery =  `INSERT INTO tmp_sewing_packing(
        line_id, buyer, orderNo, 
        style, color, size, 
        woId,cutId, inputId, 
        outputId, packDate, lineNo, 
        mpUsed, mcHrs, packPcs, 
        cartonbox, timeperiod, bundleNo, 
        createdBy, orgId
        ) values `
        var data = req.body.data;
        var i = 0;
...
// ===================== Sewing Shipping [Ends]=================================================================================================================================

// ===================== Sewing Invoice [Start]=================================================================================================================================
...
router.get('/sewing-invoice', (req, res, next) => {
    try {
        ...
        Query = `select * , date_format(invoiceDate, '%d-%m-%Y') as invoiceDate
...
router.post('/sewing-invoice', (req, res, next) => {
    try {
        ...
        var notes = req.body.notes;
        var data = [];
        var headerQuery = `INSERT INTO tmp_sewing_invoice(
        line_id, headId, fabtype, 
        fabGSM, orderNo, style, 
        color, size, woId, 
        cutId, inputId, outputId, 
        packId, shipId, shipPcs, 
        fobRate, valueUSD,remarks,createdBy, 
        orgId) values `
        var data = req.body.data;
        var i = 0;
        for (let datalist of data) {
